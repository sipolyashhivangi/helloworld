 ...
 */
class FlexscorePdo implements AuthorizationCodeInterface,
    ...
    PublicKeyInterface
{
    ...
    public function checkUserCredentials($username, $password = false, $pin = false)
    {

        if (($user = $this->getUser($username)) && $password) {
            return $this->checkPassword($user, $password);
        }
        if (($user = $this->getUser($username)) && $pin) {
    ...
    protected function checkPin($user, $pin)
    {
        return $user['pin'] == md5($pin);
    }
