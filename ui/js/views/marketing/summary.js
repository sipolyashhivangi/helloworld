...
define([
    'handlebars',
    'text!../../../html/marketing/summary.html',
    ...
    var summaryView = Backbone.View.extend({
        ...
        render: function(jsonData) {
            ...
            if(typeof(jsonData) != 'undefined' && typeof(jsonData.recoposts) != 'undefined') {
                for(var i = 0; i < jsonData.recoposts.length; i++) {
                    jsonData.recoposts[i].baseUrl = baseUrl;
                }
            }
            if(typeof(jsonData) != 'undefined' && typeof(jsonData.recopostsmore) != 'undefined') {
                for(var i = 0; i < jsonData.recopostsmore.length; i++) {
                    jsonData.recopostsmore[i].baseUrl = baseUrl;
                }
            }
            if(typeof(jsonData) != 'undefined' && typeof(jsonData.topics) != 'undefined') {
                for(var i = 0; i < jsonData.topics.length; i++) {
                    jsonData.topics[i].baseUrl = baseUrl;
                    for(var j = 0; j < jsonData.topics[i].articles.length; j++) {
                        jsonData.topics[i].articles[j].baseUrl = baseUrl;
                    }
                }
            }
            if(typeof(jsonData) != 'undefined' && typeof(jsonData.featvideos.youtube) != 'undefined') {
                for(var j = 0; j < jsonData.featvideos.youtube.length; j++) {
                    jsonData.featvideos.youtube[j].baseUrl = baseUrl;
                }
            }
            ...
            lcSummary = jsonData;
            if(lcSummary.posturl == 'learningcenter') {
                fnLearningCenterLoad();
            }
